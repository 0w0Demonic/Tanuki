...
class Tanuki_Edit extends AquaHotkey_MultiApply {
    ...
    class Caret {
         ...
         */
        Index {
            get => SendMessage(Controls.EM_GETCARETINDEX, 0, 0, this.Edit) + 1
            set => SendMessage(Controls.EM_SETCARETINDEX, --value, 0, this.Edit)
        }
    }
    ;@endregion
    ...
    /**
     * Enables paste notifications `EN_BEFORE_PASTE` and `EN_AFTER_PASTE` for
     * the edit control, on which you can register callbacks using
     * `OnBeforePaste()` and `OnAfterPaste()`.
     * 
     * To properly allow pasting, the `EN_BEFORE_PASTE` must be processed,
     * because the return value of the callback determines whether the paste
     * proceeds.
     * - `true` (nonzero) -> allow paste
     * - `false` (zero) -> prevent paste
     * 
     * `EN_AFTER_PASTE` is fired after the paste succeeds, and is purely
     * informational.
     * 
     * Requires Windows 10+.
     * 
     * @example
     * EditCtl.EnablePasteNotifs()
     * 
     * EditCtl.OnBeforePaste((EditCtl, lParam) {
     *     MsgBox("pasting: " . A_Clipboard)
     *     return true ; returning `false` prevents pasting
     * })
     * 
     * EditCtl.OnAfterPaste((EditCtl, lParam) {
     *     MsgBox("successfully pasted clipboard.")
     * })
     * 
     * @param   {Boolean?}  OnOff  turn on/off paste notifications
     */
    EnablePasteNotifs(OnOff := true) {
        SendMessage(
            Controls.EM_ENABLEFEATURE,
            EDIT_CONTROL_FEATURE.EDIT_CONTROL_FEATURE_PASTE_NOTIFICATIONS,
            !!OnOff, this)
    }

    /**
     * Enables support for enterprise data protection for the edit control.
     * 
     * @param   {Boolean?}  OnOff  turn on/off enterprise data protection
     */
    EnableEnterpriseDataProtection(OnOff := true) {
        SendMessage(
            Controls.EM_ENABLEFEATURE,
            EDIT_CONTROL_FEATURE.EDIT_CONTROL_FEATURE_ENTERPRISE_DATA_PROTECTION_PASTE_SUPPORT,
            !!OnOff, this)
    }
    ;@endregion
    ...
    ;@region Events
    /**
     * Registers a function to be called when the edit control receives focus.
     * 
    ...
    /**
     * Registers a function to be called when the edit control loses focus.
     * 
     * @param   {Func}      Fn   the function to be called
     * @param   {Integer?}  Opt  add/remove the callback
     * @returns {this}
    ...
    /**
     * Registers a function to be called when the text is being changed.
     * 
     * @param   {Func}      Fn   the function to be called
     * @param   {Integer?}  Opt  add/remove the callback
     * @returns {this}
    ...
    /**
     * Registers a function to be called when the edit control cannot
     * allocate memory. Realistically speaking, this should never be the case.
     * 
     * @param   {Func}      Fn   the function to be called
     * @param   {Integer?}  Opt  add/remove the callback
     * @returns {this}
    ...
    /**
     * Registers a function to be called when the text limit is reached.
     * 
     * @param   {Func}      Fn   the function to be called
     * @param   {Integer?}  Opt  add/remove the callback
     * @returns {this}
    ...
    /**
     * Registers a function to be called when scrolling horizontally.
     * 
     * @param   {Func}      Fn   the function to be called
     * @param   {Integer?}  Opt  add/remove the callback
     * @returns {this}
    ...
    /**
     * Registers a function to be called when scrolling vertically.
     * 
     * @param   {Func}      Fn   the function to be called
     * @param   {Integer?}  Opt  add/remove the callback
     * @returns {this}
    ...
    /**
     * Registers a function to be called after the clipboard has been pasted
     * into the edit control.
     * 
     * @example
