    ...
    ApplyTheme(Theme) {
        ...
        CustomDrawMessage(Hwnd, wParam, lParam, Msg) {
            ...
            case CDDS_POSTPAINT:
                ...
                ; find the rightmost item. It's not enough checking Count - 1 because
                ; the last item need not necessarily be the right-most item
                Rightmost := Array()
                Count := SendMessage(0x1200, 0, 0, HeaderHwnd)
                Loop Count {
                    SendMessage(0x1207, A_Index - 1, ObjGetDataPtr(LastItemRc), HeaderHwnd)
                    Rightmost.Push(LastItemRc.Right)
                }
                Rightmost := Max(Rightmost*)

                ClientRight := ClientRc.Right
                if (Rightmost < ClientRight) {
                    hDC           := nmcd.hDC
                    ClientRc.Left := Rightmost
